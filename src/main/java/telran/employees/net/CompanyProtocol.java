...
Company company;
	...
	public Response getResponse(Request request) {
		...
		try {
			...
			default -> wrongDataResponce(requestType);
			};
		} catch (Exception e) {
			response = wrongTypeResponce(e.getMessage());
		}
		return response;
	}

	private Response wrongTypeResponce(String message) {
		return new Response(ResponseCode.WRONG_REQUEST_TYPE, message);
	}

	private Response  wrongDataResponce(String requestType) {
